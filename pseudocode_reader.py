...
def Handle_Variables(lines: list, code_start: int, code_end: int) -> Tuple[List[Variable], int]:
    ...
    for counter in range(code_start, code_end):
        if lines[counter].startswith('declare'):
            ...
            variable_list.append(copy.deepcopy(new_variable))
        else: #doesn't start with declare
            line_stop = counter
            break
    return variable_list, line_stop


def Process_Output(line, variable_list):
    text = ''
    temp_line = line[6:]
...
def Find_First(text,search_elements):
    min_value = 999999
    min_element = ''
...
def Read_Variable_Value(variable_name, variable_list):
        ...
        if temp_line.startswith(tuple(ALPHA)):
            ...
            if data_type in ('string', 'char', 'real', 'integer', 'boolean'):
                ...
                if index_text.startswith(tuple(ALPHA)):
                    ...
                    array_index, index_data_type = Read_Variable_Value(index_text, variable_list)
                ...
                if v.variable_name == variable:
                    if v.variable_data_type == 'array':
                        if not v.array_index_start <= array_index <= v.array_index_end:
                            ...
                            v.variable_values[real_index] = GetInput(v.array_data_type)
...
def Get_Variable_Value(line, variable_begins, variable_list):
    ...
    value, data_type = Read_Variable_Value(variable, variable_list)
    if data_type == 'array':
...
def Main_Program(lines):
    ...
    while True: #run program
        if lines[line_stop].startswith('output'):
            Process_Output(lines[line_stop], variable_list)
        elif lines[line_stop].startswith('input'):
            variable_list = Process_Input(lines[line_stop], variable_list)
